    ...
    public void run(String... args) throws Exception {
        try {
            ...
            if (!trabajadorJpaService.findByUsername("admin").isPresent()) {
                ...
                var existingUser = userJpaService.findByUsername("admin");
                if (existingUser.isPresent()) {
                    ...
                    // MySQL: evitar duplicados con INSERT IGNORE
                    String sql = "INSERT IGNORE INTO trabajador (id, needs_password_change) VALUES (?, ?)";
                    int rowsAffected = jdbcTemplate.update(sql, existingUser.get().getId(), false);
                    ...
                    // MySQL: evitar duplicados con INSERT IGNORE en clave única username
                    String insertUserSql = "INSERT IGNORE INTO users (nombre, apellido, username, email, password, role) VALUES (?, ?, ?, ?, ?, ?)";
                    ...
                    if (userRowsAffected > 0) {
                        ...
                        var newUser = userJpaService.findByUsername("admin");
                        if (newUser.isPresent()) {
                            // Insertar en la tabla trabajador
                            String insertTrabajadorSql = "INSERT IGNORE INTO trabajador (id, needs_password_change) VALUES (?, ?)";
                            jdbcTemplate.update(insertTrabajadorSql, newUser.get().getId(), false);
                            
                            System.out.println("Usuario admin creado exitosamente con ID: " + newUser.get().getId());
                        }
                    } else {
                        System.out.println("El usuario admin ya existe en la base de datos");
                        // Intentar crear el registro de trabajador si no existe
                        var existingUserAfterConflict = userJpaService.findByUsername("admin");
                        if (existingUserAfterConflict.isPresent()) {
                            String insertTrabajadorSql = "INSERT IGNORE INTO trabajador (id, needs_password_change) VALUES (?, ?)";
                            jdbcTemplate.update(insertTrabajadorSql, existingUserAfterConflict.get().getId(), false);
